...
//		routing.joinCheck();
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		...
		info= getSucc(info);
		EventSource source= client.listenForBindings(info, this.info.id, key);
		state.addCallback(key, source);
		source.register(listener, IDHTNode.NEW_BINDING_EVENT);
		source.open();
