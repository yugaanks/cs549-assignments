...
public class ControllerClient extends Endpoint implements MessageHandler.Whole<String> {
		...
		try {
			...
			while (true) {
				try {
					...
					// TODO If we are connected, a new toplevel shell has been pushed, execute its
					// CLI.
	...
	public void onOpen(Session session, EndpointConfig config) {
		// TODO session created, add a message handler for receiving communication from
		// server.
	...
	public void onMessage(String message) {
		if (initializing) {
			if (SessionManager.ACK.equals(message)) {
				/*
				 * TODO server has accepted our remote control request, push a proxy shell on
				 * the shell stack and flag that initialization has finished (allowing the UI
				 * thread to continue). Make sure to replace the cached shell in this callback
				 * with the new proxy shell!
				 * 
	...
	protected void shutdown() throws IOException {
		 ...
		 * 1. We are still initializing when this happens (need to unblock the client
		 * thread). 2. We are running an on-going remote control session (need to remove
		 * the proxy shell). 3. The remote control session has terminated (which caused
