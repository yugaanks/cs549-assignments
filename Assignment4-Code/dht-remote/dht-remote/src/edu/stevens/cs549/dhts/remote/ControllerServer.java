...
public class ControllerServer {
	...
	public void onOpen(Session session, @PathParam("client") String client) throws IOException {
		...
		/*
		 * Try to set the current (pending) session, fail if there is already a
		 * (pending?) session.
		 */
	...
	public void onMessage(String[] commandLine) {
		if (initializing) {
			throw new IllegalStateException(
					"Communication from client before ack of remote control request: " + commandLine[0]);
		} else if (commandLine.length > 0 && IShell.QUIT.equals(commandLine[0])) {
			/*
			 * TODO Stop the current toplevel (local) shell. It is sufficient to close the
			 * session, which will trigger a callback on onClose() on both sides of the
			 * connection.
			 */
	...
	/**
	 * Stop the current local shell and remove it from the shell stack. This is only
	 * called when the shell is being remotely controlled, so the context is a
	 * proxy.
	 */
